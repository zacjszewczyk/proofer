...
exclude         = ["the", "a", "or", "my", "and", "to", "we", "I", "for", "am", "is", "are", "was", "were", "be", "being", "been", "i", "what", "of"]
...
def GenFile(iname):
    ...
    for line in iter(fd.readline, ""):
        ...
        for word in backup.split(" "):
            ...
            if (stripped.lower() in dict_count) and (stripped.lower() not in exclude):
                dict_count[stripped.lower()] += 1
            else:
                dict_count[stripped.lower()] = 1

            if (dict_count[stripped.lower()] == 3):
                # line = line.replace(" "+stripped+" ", " <span class='repeat "+stripped+"'>"+stripped+"</span> ")
        ...
        paragraph_stats = "<div class='floating_stats'><div>Word count: %d</div><div>Overused phrase: %d</div><div>Repeated: %d; Avoid: %d</div></div>" % (word_count[-1], overused_words, repeated_words, avoid_words)

        o_fd.write(paragraph_stats)
    ...
    o_fd.write(template[1] % (total, total_overused_words, total_repeated_words, total_avoid_words))
    o_fd.close()
